...
data = pd.read_csv(r'data/XBTUSD-1h-data.csv')
args = {'response_col': 'close', 'date_col': 'Date'}  # Replace 'close' and 'Date' with your columns
xgboost_model = MyXGboost(args)
xgboost_model.fit(data)
train_size = int(len(data) * 0.8)  # Use 80% of the data for training
train_data = data[:train_size]
test_data = data[train_size:]
xgboost_model.fit(train_data)
